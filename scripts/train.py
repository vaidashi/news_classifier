...
from scripts.config import logger, MLFLOW_TRACKING_URI, EFS_DIR
...
def train_step(
    ...
    for i, batch in enumerate(ds_generator):
        ...
        loss.backward()
        optimizer.step()
        total_loss += (loss.detach().item() - total_loss) / (i + 1) # running average
    return total_loss
...
def train_model(
    ...
    categories = train_ds.unique(column="Category")
    train_loop_config["num_classes"] = len(categories)
